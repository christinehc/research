...
Functions for fitting XPS spectra outputs from the UW MAF S-Probe.
"""
...
def read_xps_data_from_excel(file, sheet_name, skiprows=7, be_shift=0):
    ...
    Args:
        ...
        sheet_name: sheet (in Excel file) with P 2p data
    """
...
def plot_xps_data_matplotlib(xpsdataframe):
    ...
    """
    colors = ['black','black','red', 'red', 'blue', 'lightblue']
...
def full_xps_fit(file, sheet, name=False, skip=7, b=False, c=False, residuals=False):
    ...
    if name:
        plt.savefig(f"/mnt/c/Users/Christine/Documents/School/Work/Data/XPS/{name}.pdf", bbox_inches='tight', transparent=True)
        plt.savefig(f"/mnt/c/Users/Christine/Documents/School/Work/Data/XPS/{name}.png", bbox_inches='tight', transparent=True)
    plt.show()
